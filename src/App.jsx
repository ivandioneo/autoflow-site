...
// Webhook URL from environment — set VITE_WEBHOOK_URL in Cloudflare Pages env vars.
// Falls back to empty string so the form fails gracefully rather than hitting a
// hardcoded URL if the variable is missing.
...
function SavingsCalculator() {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ color: "white", fontSize: "clamp(1.1rem, 1rem + 0.5vw, 1.375rem)", fontFamily: "var(--font-display)", fontWeight: 800, marginBottom: 4, letterSpacing: -0.5 }}>
      ...
      {[
        ...
        <div key={i} style={{ marginBottom: 22 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "clamp(0.75rem, 0.7rem + 0.25vw, 0.8125rem)", fontWeight: 500 }}>{s.label}</span>
            <span style={{ color: "#F59E0B", fontSize: "clamp(0.8125rem, 0.75rem + 0.3vw, 0.9375rem)", fontWeight: 800 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "clamp(0.625rem, 0.6rem + 0.15vw, 0.6875rem)", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>You're losing</div>
          <div style={{ color: "#EF4444", fontSize: "clamp(1.25rem, 1rem + 1vw, 1.75rem)", fontWeight: 800, marginTop: 4 }}>
            {lostPerMonth.toLocaleString()} <span style={{ fontSize: "clamp(0.8125rem, 0.75rem + 0.3vw, 0.875rem)", fontWeight: 600 }}>AED/mo</span>
          </div>
        </div>
        <div style={{
          ...
          <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "clamp(0.625rem, 0.6rem + 0.15vw, 0.6875rem)", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>We save you</div>
          <div style={{ color: "#10B981", fontSize: "clamp(1.25rem, 1rem + 1vw, 1.75rem)", fontWeight: 800, marginTop: 4 }}>
            {savedPerMonth.toLocaleString()} <span style={{ fontSize: "clamp(0.8125rem, 0.75rem + 0.3vw, 0.875rem)", fontWeight: 600 }}>AED/mo</span>
          </div>
        </div>
      </div>
      <div style={{
        ...
        <span style={{ color: "#F59E0B", fontSize: "clamp(1.125rem, 0.9rem + 0.9vw, 1.375rem)", fontWeight: 800 }}>{savedPerYear.toLocaleString()} AED</span>
...
function PricingCard({ tier, price, desc, features, highlight, badge, onCta, ctaLabel }) {
  ...
  return (
    ...
    >
      {badge && (
        <div style={{
          // Amber accent reserved only for the Most Popular badge — nowhere else in pricing
      ...
      <div style={{ fontSize: "clamp(1rem, 0.9rem + 0.4vw, 1.25rem)", fontFamily: "var(--font-display)", fontWeight: 800, color: highlight ? "white" : "#0F172A" }}>{tier}</div>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
        <span style={{ fontSize: "clamp(1.75rem, 1.2rem + 2vw, 2.5rem)", fontFamily: "var(--font-display)", fontWeight: 900, color: highlight ? "white" : "#0F172A", letterSpacing: -1 }}>{price}</span>
      ...
      <button
        ...
        style={{
          ...
          // Teal CTA on all cards — amber gradient reserved for the badge only
          background: "#0D9488",
          color: "white", fontWeight: 700, fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.9375rem)", cursor: "pointer",
...
export default function AutoFlowLanding() {
  ...
  // Tightened: E.164-style, 7-15 digits, no leading zero after optional +
  ...
  const validate = () => {
    ...
    if (!form.whatsapp || !isValidWhatsApp(form.whatsapp)) e.whatsapp = "Enter a valid WhatsApp number (e.g. 971501234567)";
  ...
  const handleSubmit = async () => {
    ...
    if (!validate()) return;
    if (!WEBHOOK_URL) {
      setErrors({ submit: "Service configuration error. Please contact us directly." });
      return;
    }
    setSubmitting(true);
    ...
    try {
      ...
      // mode: 'no-cors' — we don't need to read the response body;
      // referrerPolicy: 'no-referrer' — don't leak the page URL to the webhook host.
  ...
  return (
    <div style={{
      ...
      color: "#334155", background: "#F0F4F8", overflowX: "hidden",
    }}>
      <style>{`
        :root {
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Float only when motion is okay — no CPU burn on reduced-motion */
        @media (prefers-reduced-motion: no-preference) {
          .phone-float { animation: float 6s ease-in-out infinite; }
        }
        /* Hide phone mockup on small screens to avoid layout overflow */
        ...
        @media (max-width: 680px) {
          ...
          .hero-h1 { font-size: clamp(2rem, 5vw + 1rem, 3.25rem) !important; }
        }
      ...
      {/* ─── SKIP LINK ─── */}
      <a href="#main" style={{
        position: "absolute", top: -40, left: 0, padding: "8px 12px",
        background: "#0D9488", color: "white", fontWeight: 700, fontSize: 13,
        zIndex: 9999, borderRadius: "0 0 8px 0",
        transition: "top 0.2s",
      }}
      ...
      <nav aria-label="Main navigation" style={{
        ...
        {/* Logo */}
        <a href="/" aria-label="AutoFlow home" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <div style={{
          ...
          <span style={{ fontFamily: "var(--font-display)", fontWeight: 400, fontSize: 20, color: "white", letterSpacing: -0.3 }}>AutoFlow</span>
        </a>
        ...
        <button
          ...
          style={{
            color: "white", flexDirection: "column", gap: 5, padding: 8,
            background: "none", border: "none", cursor: "pointer",
          }}
        >
          <span style={{ width: 22, height: 2, background: "white", borderRadius: 2, display: "block", transition: "transform 0.2s", transform: mobileMenuOpen ? "rotate(45deg) translate(5px,5px)" : "none" }} />
          <span style={{ width: 22, height: 2, background: "white", borderRadius: 2, display: "block", opacity: mobileMenuOpen ? 0 : 1, transition: "opacity 0.2s" }} />
          <span style={{ width: 22, height: 2, background: "white", borderRadius: 2, display: "block", transition: "transform 0.2s", transform: mobileMenuOpen ? "rotate(-45deg) translate(5px,-5px)" : "none" }} />
        </button>
      </nav>
      ...
      {mobileMenuOpen && (
        <div style={{
          ...
          {["#calc:Calculator", "#how:How it works", "#pricing:Pricing"].map(item => {
            const [href, label] = item.split(":");
            return (
              <a key={href} href={href} onClick={() => setMobileMenuOpen(false)}
                style={{ color: "rgba(255,255,255,0.8)", textDecoration: "none", fontWeight: 500, fontSize: 16 }}>
                {label}
              </a>
            );
          })}
          <a href={DASHBOARD_URL} style={{ color: "rgba(255,255,255,0.6)", textDecoration: "none", fontWeight: 500, fontSize: 16 }}>Log in</a>
      ...
      <section style={{
        ...
        {/* Ambient glows — kept subtle, no animation to avoid CPU waste */}
        ...
        <div style={{
          ...
          <div style={{ flex: "1 1 420px", maxWidth: 540 }}>
            <div style={{
              ...
              <div aria-hidden="true" style={{ width: 6, height: 6, borderRadius: 3, background: "#F59E0B", animation: "dotPulse 2s infinite" }} />
            ...
            <p style={{
              fontSize: "clamp(0.9375rem, 0.85rem + 0.4vw, 1.0625rem)",
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 36 }}>
              {/* Primary hero CTA — teal, not amber (amber reserved for nav Get a Quote) */}
              <a href="#quote" style={{
                background: "linear-gradient(135deg, #0D9488, #14B8A6)",
                color: "white", padding: "16px 32px", borderRadius: 14,
                textDecoration: "none", fontWeight: 700, fontSize: "clamp(0.9375rem, 0.85rem + 0.4vw, 1rem)",
              ...
              <a href="#calc" style={{
                ...
                textDecoration: "none", fontWeight: 600, fontSize: "clamp(0.875rem, 0.8rem + 0.35vw, 0.9375rem)",
              }}>Calculate Your Savings ↓</a>
            </div>
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)", color: "rgba(255,255,255,0.35)" }}>
          ...
          {/* Phone: hidden on mobile via CSS, float only when motion is allowed */}
          <div className="phone-float" style={{ flex: "0 0 auto" }}>
            <PhoneMockup />
          </div>
        </div>
      </section>
        ...
        <FadeIn>
          <div style={{ maxWidth: 1140, margin: "0 auto", display: "flex", flexWrap: "wrap", gap: 48, alignItems: "center", justifyContent: "center" }}>
            <div style={{ flex: "1 1 340px", maxWidth: 440 }}>
              <h2 style={{
                fontSize: "clamp(1.5rem, 1rem + 1.5vw, 2.125rem)",
                fontFamily: "var(--font-display)",
                fontWeight: 400, color: "white", lineHeight: 1.15, letterSpacing: -0.5, marginBottom: 16,
              }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", gap: 12, alignItems: "center" }}>
                    ...
                    <span style={{ color: "rgba(255,255,255,0.6)", fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.875rem)" }}>{p.text}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {[
            ...
            <div key={i} style={{ textAlign: "center", minWidth: 160 }}>
              <div style={{
                ...
                fontFamily: "var(--font-display)",
                fontWeight: 400, color: "#0F172A", letterSpacing: -1,
              }}>
                {s.n === 0 ? "0" : <Counter target={s.n} />}{s.s}
              </div>
              <div style={{ fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)", color: "#94A3B8", marginTop: 4, fontWeight: 500 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
          ...
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 48 }}>
              <h2 style={{
                fontSize: "clamp(1.5rem, 1rem + 1.5vw, 2.125rem)",
                fontFamily: "var(--font-display)",
                fontWeight: 400, color: "#0F172A", letterSpacing: -0.5,
              }}>How it works</h2>
            ...
            {/* Step timeline — left-bordered steps replace icon-in-circle grid */}
            <div style={{ display: "flex", flexDirection: "column", gap: 0, maxWidth: 640, margin: "0 auto" }}>
              {[
                ...
                { num: "02", title: "Flow triggers",    desc: "Our automation engine picks it up instantly — no delay, no manual entry needed.",                  color: "#0D9488" },
                ...
                <div key={i} style={{ display: "flex", gap: 24, position: "relative" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                    <div style={{
                      ...
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: "clamp(0.65rem, 0.6rem + 0.2vw, 0.75rem)", fontWeight: 800, color: "#0D9488", letterSpacing: 0.5,
                      flexShrink: 0,
                    }}>{s.num}</div>
                    {i < arr.length - 1 && (
                      <div style={{ width: 1, flex: 1, minHeight: 32, background: "rgba(13,148,136,0.15)", margin: "4px 0" }} />
                    )}
                  </div>
                  <div style={{ paddingBottom: i < arr.length - 1 ? 32 : 0, paddingTop: 8 }}>
                    <div style={{ fontWeight: 700, fontSize: "clamp(0.9375rem, 0.85rem + 0.35vw, 1.0625rem)", color: "#0F172A", marginBottom: 4 }}>{s.title}</div>
                    <div style={{ fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.9375rem)", color: "#64748B", lineHeight: 1.6 }}>{s.desc}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section style={{ padding: "72px 32px", background: "white" }}>
        <FadeIn>
          <div style={{ maxWidth: 1000, margin: "0 auto" }}>
            <h2 style={{
              fontSize: "clamp(1.375rem, 1rem + 1.25vw, 1.875rem)",
              fontFamily: "var(--font-display)",
              fontWeight: 400, color: "#0F172A", textAlign: "center", marginBottom: 12, letterSpacing: -0.3,
            }}>
              Built for businesses like yours
            </h2>
            <p style={{ color: "#94A3B8", textAlign: "center", fontSize: "clamp(0.875rem, 0.8rem + 0.35vw, 0.9375rem)", marginBottom: 40 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 16, justifyContent: "center" }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  <div style={{ fontWeight: 700, fontSize: "clamp(0.875rem, 0.8rem + 0.3vw, 0.9375rem)", color: "#0F172A", marginBottom: 6 }}>{c.title}</div>
                  <div style={{ fontSize: "clamp(1.1rem, 0.9rem + 0.7vw, 1.5rem)", fontFamily: "var(--font-display)", fontWeight: 400, color: "#0D9488", marginBottom: 6 }}>{c.stat}</div>
                  <div style={{ fontSize: "clamp(0.7rem, 0.65rem + 0.2vw, 0.8125rem)", color: "#64748B", lineHeight: 1.5 }}>{c.desc}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section id="pricing" style={{ padding: "80px 32px", background: "#F8FAFC" }}>
        <FadeIn>
          <div style={{ maxWidth: 1060, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 20, justifyContent: "center", alignItems: "flex-end" }}>
            ...
            <p style={{ textAlign: "center", color: "#94A3B8", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)", marginTop: 28 }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 280 }}>
              <div style={{
                ...
                fontFamily: "var(--font-display)",
                fontWeight: 400, color: "white", marginBottom: 6,
              }}>
                Built security-first, from the ground up
              </div>
              <div style={{ fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.875rem)", color: "rgba(255,255,255,0.45)", lineHeight: 1.7 }}>
          ...
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            ...
            {submitted ? (
              <div style={{
                ...
                <div style={{ fontSize: "clamp(1.125rem, 0.9rem + 0.9vw, 1.375rem)", fontFamily: "var(--font-display)", fontWeight: 400, color: "#0F172A", marginBottom: 8 }}>
                  You're in!
                </div>
                <div style={{ color: "#64748B", fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.875rem)", lineHeight: 1.6 }}>
              ...
              <div style={{
                ...
                {errors.submit && (
                  <div role="alert" style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 10, padding: "10px 14px", marginBottom: 14, color: "#DC2626", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)" }}>
                    {errors.submit}
                  </div>
                )}
                <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: 14 }}>
                  {[
                    ...
                    <div key={f.key} style={{ flex: f.w }}>
                      <label htmlFor={`field-${f.key}`} style={{ fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.75rem)", fontWeight: 700, color: "#0F172A", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.3 }}>
                        {f.label}
                      </label>
                      <input
                        id={`field-${f.key}`}
                        ...
                        style={{
                          ...
                          fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.875rem)", outline: "none",
                          fontFamily: "inherit", background: "white", transition: "all 0.2s",
                        }}
                      />
                      {errors[f.key] && (
                        <p id={`err-${f.key}`} role="alert" style={{ color: "#EF4444", fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.6875rem)", marginTop: 4 }}>{errors[f.key]}</p>
                      )}
                    </div>
                  ))}
                </div>
                <div style={{ marginBottom: 14 }}>
                  <fieldset style={{ border: "none", padding: 0 }}>
                    <legend style={{ fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.75rem)", fontWeight: 700, color: "#0F172A", display: "block", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.3 }}>
                      Business type
                    </legend>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {["Salon", "Clinic", "Tutor", "Spa", "Travel", "Other"].map(t => (
                        <button
                          ...
                          style={{
                            padding: "8px 16px", borderRadius: 10, fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)", fontWeight: 600,
                  ...
                  {errors.type && (
                    <p role="alert" style={{ color: "#EF4444", fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.6875rem)", marginTop: 4 }}>{errors.type}</p>
                  )}
                </div>
                <div style={{ marginBottom: 18 }}>
                  <label htmlFor="field-message" style={{ fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.75rem)", fontWeight: 700, color: "#0F172A", display: "block", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.3 }}>
                  ...
                  <textarea
                    ...
                    style={{
                      width: "100%", padding: "12px 14px", borderRadius: 10,
                      border: "1px solid #E2E8F0", fontSize: "clamp(0.8125rem, 0.75rem + 0.25vw, 0.875rem)", outline: "none",
                      fontFamily: "inherit", resize: "vertical", background: "white",
                      transition: "all 0.2s",
                    }}
                  />
                  <p style={{ textAlign: "right", fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.6875rem)", color: "#94A3B8", marginTop: 2 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: "white", fontWeight: 800, fontSize: "clamp(0.9375rem, 0.85rem + 0.35vw, 1rem)",
                ...
                <p style={{ textAlign: "center", fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.75rem)", color: "#94A3B8", marginTop: 10 }}>
      ...
      <footer style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginBottom: 16 }}>
          ...
          <span style={{ fontFamily: "var(--font-display)", fontWeight: 400, fontSize: 18, color: "white" }}>AutoFlow</span>
        </div>
        <nav aria-label="Footer navigation" style={{ display: "flex", justifyContent: "center", gap: 20, marginBottom: 16 }}>
          <a href="#pricing" style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)" }}>Pricing</a>
          <a href="#quote"   style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)" }}>Get a Quote</a>
          <a href={DASHBOARD_URL} style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)" }}>Log in</a>
        </nav>
        <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "clamp(0.75rem, 0.7rem + 0.2vw, 0.8125rem)", marginBottom: 6 }}>
          Automation services for businesses in the UAE
        </div>
        <div style={{ color: "rgba(255,255,255,0.2)", fontSize: "clamp(0.65rem, 0.6rem + 0.15vw, 0.75rem)" }}>© 2026 AutoFlow. All rights reserved.</div>
      </footer>
      </main>
    </div>
  );
